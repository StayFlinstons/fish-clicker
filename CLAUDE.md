...
- **Feature modules** — each exports a class whose methods become `FishingGame` methods (`this` is the game; `window.game.x()` and HTML `onclick` keep working). `core/mixins.js` throws on load if two modules define the same method name.
  ...
  - `systems/` — `fishing.js` (`fish()`, `recordDiscovery()`, selling, bucket/aquarium), `shop.js`, `automation.js` (auto-fisher/auto-seller), `offline.js`, `fishEyes.js` (daily Fish Eyes, Sanctuary, species offerings), `magnet.js` (game modes + Pesca Magnética/Forge/Museum), `depth.js` (depth layers: layer selector, per-layer scenery, unlocking a layer when a rod is bought, one-time notice after migrating an old save), `tools.js` (Sonar and Quadro de Encomendas upgrades), `chapter1.js` (portal/altar/Kraken; landing the Kraken is a one-time Mythic catch, not a world change), `achievements.js`, `events.js` (Golden Fish, Blood Moon Eclipse, temp buffs).
  - `ui/` — `render.js` (`renderAll()` and fishing-mode panels), `sprites.js` (fish sprite cache), `mobileNav.js` (mobile tab bar + header gold), `lakeBackground.js` (animates the layer 1 (river) background PNGs: surface ripple + swaying plants by re-drawing horizontal strips shifted in whole art blocks, per-image bands in `BANDS_BY_IMAGE`; off with the "water particles" setting), `effects.js` (toasts, floating text, catch popups), `celebrations.js`, `album.js`, `summary.js`, `settings.js`, `patchNotes.js`.
...
- **`fishData.js`** — `RARITIES` (7 tiers: COMUM → SECRETO, each with drop `chance` and color) and `FISH_LIST` (species catalog: id, rarity, weight range, baseValue, optional `buff` for aquarium bonuses, `layer` = depth layer 1–6, optional `timeExclusive` for day/sunset/night-only fish (only applies in sunlit layers), optional `migratesUp` for layer-3 fish that also show up in layer 2 at night). Weights are real kg; `baseValue` grows with rarity and layer.
...
- **`depthData.js`** — `DEPTH_LAYERS`: the 6 depth layers (Rio e Lagoa → Fossa Hadal) with depth range, `sunlit` (day/night cycle applies), scenery and colors. There is one world; the player picks any layer up to `getMaxLayer()` (deepest owned rod). Old World 2 saves are merged by `migrateToDepthLayers()` in `core/save.js` (flag `depthLayersVersion`).
