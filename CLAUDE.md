...
- **Feature modules** — each exports a class whose methods become `FishingGame` methods (`this` is the game; `window.game.x()` and HTML `onclick` keep working). `core/mixins.js` throws on load if two modules define the same method name.
  - `core/` — `constants.js` (`GAME_VERSION`, `SAVE_KEY`), `save.js` (save/load/reset/export-import), `economy.js` (buffs, rarity chances, weight/value, `rollFish()`).
  - `systems/` — `fishing.js` (`fish()`, `recordDiscovery()`, selling, bucket/aquarium), `shop.js`, `automation.js` (auto-fisher/auto-seller), `offline.js`, `fishEyes.js` (daily Fish Eyes, Sanctuary, species offerings), `magnet.js` (game modes + Pesca Magnética/Forge/Museum), `depth.js` (depth layers: layer selector, per-layer scenery, unlocking a layer when a rod is bought, one-time notice after migrating an old save), `tools.js` (Quadro de Encomendas upgrade), `achievements.js`, `events.js` (Golden Fish, Blood Moon Eclipse, temp buffs).
  - `ui/` — `render.js` (`renderAll()` and fishing-mode panels), `sprites.js` (fish sprite cache), `mobileNav.js` (mobile tab bar + header gold), `pixelEmoji.js` (swaps every emoji rendered in the page for an 8x8 pixel-art SVG via a MutationObserver; game text keeps plain emoji, add a drawing to `ICONS` for a new one), `lakeBackground.js` (animates the layer background PNGs: surface ripple, swaying plants, smoke, heat shimmer by re-drawing horizontal strips shifted in whole art blocks, per-image bands in `BANDS_BY_IMAGE`; off with the "water particles" setting), `effects.js` (toasts, floating text, catch popups), `celebrations.js`, `album.js`, `summary.js`, `settings.js`, `patchNotes.js`.
...
- **Methods marked "Sem chamadas no momento"** belong to features paused on purpose (magnet mode entry). Keep them.
- **`fishData.js`** — `RARITIES` (7 tiers: COMUM → SECRETO, each with drop `chance` and color) and `FISH_LIST` (species catalog: id, rarity, weight range, baseValue, optional `buff` for aquarium bonuses, `layer` = depth layer 1–6). Weights are real kg; `baseValue` grows with rarity and layer.
- **`itemsData.js`** — `RODS` (each has `depthLayer`, the deepest layer it reaches, and `maxWeight`, the PWR in kg; fish heavier than PWR can escape, see `getLandChance()`), `BAITS` (all the luck/speed/double-catch buffs come from the bait), `UPGRADES` shop catalogs and `isCosmicOrHigherRod()` helper.
- **`depthData.js`** — `DEPTH_LAYERS`: the 6 depth layers (Rio e Lagoa → Fossa Hadal) with depth range, background `image` (`zonas/zona_N.png`), `scenery` (sea ambience in the water renderer), `fishStyle` (look of the background minifish: sunlit/photophores/glow/charred/ghost, drawn in `_drawMinifish()` in `pixelArt.js`) and colors. There is one world; the player picks any layer up to `getMaxLayer()` (deepest owned rod). Old World 2 saves are merged by `migrateToDepthLayers()` in `core/save.js` (flag `depthLayersVersion`).
...
- **`sw.js` / `manifest.json`** — Service Worker (offline cache) and PWA manifest.
- **Versioning / shipping a change:** bump `ASSET_VERSION` (inline script at the end of `index.html`) **and** `CACHE_NAME` in `sw.js` to the same number. That script builds an import map so every module is requested as `file.js?v=ASSET_VERSION` (no mixing of new `game.js` with old cached modules after a deploy) and then loads `game.js`. New `.js` files must be added to its `MODULES` list; `node scripts/check_modules.cjs` checks both the list and that the two versions match.
...
Same DOM for both, no duplicated markup (code finds elements by id). At ≥1024px the `<main>` grid shows three columns (shop | scene | bucket). Below 1024px (`style.css`, "LAYOUT MOBILE") only one panel is shown at a time: the three panels carry `data-mpanel="loja|pescar|balde"`, `body[data-mobile-tab]` picks the visible one, and the `#mobile-nav` tab bar switches it via `setMobileTab()`. Elements with `.mobile-only` exist only in the mobile layout. Active buffs (including the Golden Fish timers) are rendered by `renderBuffs()` into both the header (desktop) and `#mobile-buffs-list` (mobile, between the lake and the stats bar). Toasts and achievement popups go through `pushNotice()` (`ui/effects.js`): one stack at the top, below the header (max 3; tap anywhere on a notice or its ✕ to close; a `key` dedupes identical notices). Fish-catch popups (`#catch-toast-container`, can be turned off in settings) stay at the top of the lake and are pushed below that stack by `layoutNotices()`. Minimum text size is 8px (Press Start 2P's native grid; smaller sizes blur).
...
There is no day/night cycle: each layer has one background image (`zonas/zona_N.png`, set by `applyLayerScenery()` in `systems/depth.js`). The Red Eclipse swaps layer 1 to `fundo-lago-eclipse.png` and tints the other layers red.
...
`initConsole()`/`execConsoleCmd()` implement a debug console (toggle with `Ctrl+Shift+'`, close with `Esc`) with commands for adding gold, spawning events, unlocking content, and simulating offline time — run `help` in it for the full list. This is the fastest way to manually verify a change without grinding the economy.
